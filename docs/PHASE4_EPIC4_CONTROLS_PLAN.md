...
Phases 1–3 established tokens, layout/typography primitives, and a persistent app
shell. Phase 4 introduces the **interactive control layer**: buttons and form
...
inside the Phase 3 shell without inventing one-off styles.
...
1. Merge Phase 3 (`phase3-app-shell` / PR #7) into `main`.
2. Pull latest `main`; branch `phase4-controls`.
...
4. Skim [PRIMITIVES.md](./PRIMITIVES.md) conventions (`as`, no style/class inputs,
   OnPush, token maps) — Phase 4 must match them.
...
Without Phase 4, Phase 5 data tables and Phase 6 overlays have nothing consistent
to trigger or edit.
...
| Table bulk actions patterns              | Phase 5 consumes buttons; doesn’t own them                         |
| Visual regression baselines              | Phase 8                                                            |
| Replacing shell chrome buttons mid-phase | Optional follow-up once `jp-icon-button` exists                    |
...
- [ ] New route e.g. `/phase-4-controls` (keep prior phase routes)
...
- [ ] Root redirect may stay on phase-3 until this story lands; then optionally point to phase-4
...
| Showcase route         | Replace phase-3 vs add phase-4             | Add `/phase-4-controls`; keep phase-3               |
...
3. [JP_ROADMAP.md](./JP_ROADMAP.md) Phase 4 marked complete.
...
settings/filter UIs on top of the Phase 3 shell.
...
1. Merge Phase 3; branch `phase4-controls` from `main`.
