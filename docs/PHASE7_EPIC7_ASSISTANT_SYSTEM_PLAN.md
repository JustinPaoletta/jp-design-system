# Phase 7 Epic 7 — Assistant System Implementation Plan

**Status:** Complete — Stories 7.0–7.5 delivered  
**Branch:** `phase7-assistant-system`  
**Roadmap:** [JP_ROADMAP.md](./JP_ROADMAP.md) · **Principles:** [DESIGN_PRINCIPLES.md](./DESIGN_PRINCIPLES.md)  
**Depends on:** Phase 6 Feedback & Overlays complete ([PHASE6_EPIC6_FEEDBACK_OVERLAYS_PLAN.md](./PHASE6_EPIC6_FEEDBACK_OVERLAYS_PLAN.md))  
**PR base:** `phase6-feedback-overlays` (PR #10) until that chain merges to `main`

---

## Executive summary

Phases 1–6 established tokens, layout/typography, an app shell, controls, data
display, and a full feedback/overlay layer. Phase 7 introduces the **branded
...
to ship a branded assistant integration inside the Phase 3 shell using Phase 4
controls and Phase 6 overlays for confirm/dismiss patterns.
...
1. Phase 6 (`phase6-feedback-overlays` / PR #10) is the tip of the open phase chain.
2. Branch `phase7-assistant-system` from `phase6-feedback-overlays` (not from
   `main` while the chain is unmerged).
3. Confirm coverage gates remain green (`ui` / `showcase` ≥ 90%).
4. Skim [PRIMITIVES.md](./PRIMITIVES.md) conventions — Phase 7 must match them.
...
Without Phase 7, Showcase cannot prove branded assistant integration, and product
...
Phase 7 includes an explicit **tone refinement** for assistant surfaces:
...
| Showcase route    | Replace phase-6 vs add phase-7    | Add `/phase-7-assistant`; keep phase-6; root → phase-7 |
| PR granularity    | One epic PR vs per-component      | Single epic PR chained on `phase6-feedback-overlays`   |
...
3. [JP_ROADMAP.md](./JP_ROADMAP.md) Phase 7 marked complete.
...
7. Phase 8 was **not** started.

**Deliverable:** Branded assistant integration — service, context trigger, message
tones, and panel ready for product assistant surfaces on top of Phases 3–6.
...
| ------- | -------------------------------------------------------------- | -------------------------- |
| 7.0–7.5 | Tokens + service + trigger + message + panel + Showcase + docs | `phase6-feedback-overlays` |

Single epic PR is preferred for Phase 7 given the tightly coupled assistant set.

---

## First implementation steps

1. Branch `phase7-assistant-system` from `phase6-feedback-overlays`.
2. Story 7.0: add assistant tokens; lock API decisions below.
3. Implement service + trigger, then message, then panel.
4. Showcase `/phase-7-assistant` + composition story + e2e + docs.
5. Open PR with base `phase6-feedback-overlays`.
...
| Showcase route  | `/phase-7-assistant`; keep prior routes; root redirect → phase-7 |
| LLM / streaming | Deferred                                                         |
| Phase 8         | Not started in this epic                                         |
